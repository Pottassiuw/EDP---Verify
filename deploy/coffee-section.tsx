/* EDP Verify — Seção COFFEE.
   Abertura de notas direto no COFFEE, em lote. O usuário digita os IDs um a um
   (campo vira chips); IDs inválidos/duplicados são descartados automaticamente.
   Três modos de abertura, à escolha: todas de uma vez · em blocos · lista de
   links. Rastreia o que já foi aberto na sessão (não reabre sem querer).
   Dois layouts (composer / split) alternáveis via Tweak. Estado persiste em
   localStorage para sobreviver à troca de seção e a recarregamentos. */
...
const COFFEE_STYLE = `
  ...
  .coffee-fb b{color:var(--accent);font-weight:600}
...
const CoffeeSection: React.FC<CoffeeSectionProps> = ({ notes, layout }) => {
...
window.CoffeeSection = CoffeeSection;
