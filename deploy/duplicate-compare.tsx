/* EDP Verify — comparação de duplicatas.
   Mostra a nota aberta lado a lado com cada candidata, destacando os campos-chave
   (local · ID SAP · descrição · poste) que coincidem, e permite abrir cada uma —
   ou todas — direto no COFFEE para confirmar antes de marcar como duplicata. */
...
const dupcEq = (a: string, b: string): boolean => dupcNorm(a) === dupcNorm(b);

const DuplicateCompare: React.FC<DuplicateCompareProps> = ({ note, resolved, onMarkDuplicate }) => {
  const cands = note.duplicates;
  if (!cands.length) return null;
  const api = window.EDPApi;
  ...
  return (
    <section>
      ...
      {cands.map((c) => {
        const matches = DUPC_KEYS.filter((f) => dupcEq(note[f.key], c[f.key])).length;
        const strong = matches === DUPC_KEYS.length;
        return (
            ...
            <div className="dupc-hd">
              ...
              <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                {c.latitude && c.longitude && (
                  <a className="edp-btn sm" target="_blank" rel="noopener" href={api.mapsUrl(c.latitude, c.longitude)}
...
window.DuplicateCompare = DuplicateCompare;
