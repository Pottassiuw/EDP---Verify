...
function normalize(j: ApiData): FetchResult {
  ...
  const notes: Note[] = records.map((r): Note => {
    ...
    return {
      ...
      errors: r.errors ?? [],
      status: r.status,
      duplicates: (r.duplicates ?? []).map((d) => ({ ...d, in_sheet: d.in_sheet ?? false })),
      raw: raw as NoteRaw,
    };
  });
...
export async function upload(file: File): Promise<UploadResult> {
  ...
  if (!res.ok) throw await erroComDetail(res, "POST /upload");
...
export async function consultarNota(id: number): Promise<import('./features/coffee/types').CoffeeConsulta> {
  ...
  if (!res.ok) throw await erroComDetail(res, "GET /consultar");
  return res.json();
}
