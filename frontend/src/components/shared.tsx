...
import type { RuleKey, LogoProps, StatTileProps, DonutProps, RuleBreakdownProps, FieldProps, NoteStatus } from '../types';
import { ruleMeta } from '../api';
import { EDP } from '../data';
...
export function prioMeta(p: number): [string, string | number] {
  if (p >= 99) return ["none", "—"];
...
export const StatusTag: React.FC<{ status: NoteStatus; done: boolean; dup?: boolean }> = ({ status, done, dup }) => {
...
export const RuleTag: React.FC<{ rule: RuleKey }> = ({ rule }) => {
  const meta = ruleMeta(rule);
  return <span className="edp-rule">{meta.short}</span>;
};

export const StatTile: React.FC<StatTileProps> = ({ label, value, accent = "green", sub, big }) => {
...
export const Donut: React.FC<DonutProps> = ({ pct, size = 92, stroke = 11, color = "var(--green)" }) => {
...
export const RuleBreakdown: React.FC<RuleBreakdownProps> = ({ stats, max, compact }) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: compact ? 7 : 10 }}>
      {entries.map(([rule, n], i) => {
        const meta = EDP.RULES[rule] ?? { label: rule };
...
export const Field: React.FC<FieldProps> = ({ label, accent, children, grow }) => (
